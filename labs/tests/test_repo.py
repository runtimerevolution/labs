...
class TestApplyCodeChanges(unittest.TestCase):
    ...
    example_single_step: List[Step] = [Step(type="modify", path="/path/to/modify/file", content="file content")]
    example_multiple_steps: List[Step] = [
        Step(type="create", path="/path/to/create/file", content="file content"),
        Step(type="modify", path="/path/to/modify/file", content="file content"),
    ]

    @patch("repo.modify_file", return_value="/path/to/modify/file")
    @patch("repo.parse_llm_output", return_value=Response(steps=example_single_step))
    ...
    @patch("repo.parse_llm_output", return_value=Response(steps=example_multiple_steps))
    ...
    @patch("repo.parse_llm_output", return_value=Response(steps=example_no_step))
