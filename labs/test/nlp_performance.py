...
spacy_df["quality"] = spacy_df.apply(
    lambda row: evaluate_summary_quality(row["reference"], row["summary"]), axis=1
)
hf_df["quality"] = hf_df.apply(
    lambda row: evaluate_summary_quality(row["reference"], row["summary"]), axis=1
)

# Combine results for comparison
combined_df = pd.concat(
    [spacy_df.assign(library="spacy"), hf_df.assign(library="huggingface")]
)
